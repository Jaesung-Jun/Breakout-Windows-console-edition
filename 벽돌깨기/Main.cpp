				...
				while (1) {					//Game Start!!
					...
					if (game_over == FALSE && key.Game_Pause_Check()) { //Pause Game
						...
						while (1) {
							...
							screen.Print_Crashed_Block_Num(score_box.xy, player.destroyed_block);
							screen.Print_Player_Score(score_box.xy, player.score);
							screen.Print_Remain_Block_Num(score_box.xy, wall.nblocks - player.destroyed_block);
