...
using namespace std;
DoubleBuffering::DoubleBuffering(short window_x, short window_y) {
...
void DoubleBuffering::Write_Buffer(COORD CursorPosition, string str) {
	DWORD dw;
...
void DoubleBuffering::Flip_Buffer(short delay) {
	Sleep(delay);
	SetConsoleActiveScreenBuffer(hBuffer[screen_index]);
	screen_index = !screen_index;
}

void DoubleBuffering::CleanUp_Buffer(short window_x, short window_y) {
	COORD coord = { 0, 0 };
	DWORD dw;
	FillConsoleOutputCharacter(hBuffer[screen_index], ' ', window_x*window_y, coord, &dw);
}

void DoubleBuffering::Disable_Buffer() {
	CloseHandle(hBuffer[0]);
	CloseHandle(hBuffer[1]);
}

void DoubleBuffering::Move_Cursor(COORD CursorPosition) {
